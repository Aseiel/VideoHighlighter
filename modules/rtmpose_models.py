...
``modules.pose_backend.find_default_rtmpose_ir`` looks. Mirrors
``modules/yolox_models.py`` deliberately: same cache-then-convert shape, same
...
MODEL_DIR = Path(__file__).resolve().parent.parent / "models" / "rtmpose"
