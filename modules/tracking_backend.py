...
from modules.detection_backend import YoloxOpenVINODetector, find_default_yolox_ir
...
def resolve_yolox_ir(model_size: str = "n") -> str | None:
    ...
    explicit = Path(__file__).resolve().parent.parent / "models" / "yolox" / f"yolox_{suffix}.xml"
    if explicit.is_file():
        return str(explicit)
