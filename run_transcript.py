...
def extract_word_timestamps(segments: List[Dict]) -> List[Dict]:
    ...
    for seg in segments:
        if 'words' in seg and seg['words']:
            # Use the word-level timestamps if available (Whisper >= 2023)
            ...
            # Fallback: split segment into words with estimated timestamps
...
# --------------------------
# SRT & TRANSCRIPT GENERATION (UPDATED WITH WORD-LEVEL TIMING)
# --------------------------

def create_srt_content(segments, max_chars=80, min_duration=0.8, pause_threshold=1.0):
    """
    Create SRT content with word-level timing for better synchronization
    """
    srt_lines = []
    subtitle_number = 1
    ...
    if not word_segments:
        # Fallback to segment-level timing if no word timestamps available
        return create_srt_content_fallback(segments, max_chars, min_duration)
    ...
    for i, word_seg in enumerate(word_segments):
        ...
        if len(word_seg['text']) < 2 and not word_seg['text'].isalnum():
            continue
        ...
        if current_start is None:
            ...
            current_text = word_seg['text']
            continue
        
        # Check pause between words
        gap = word_seg['start'] - current_end
        
        # Check if adding this word would exceed max characters or there's a significant pause
        if (len(current_text + " " + word_seg['text']) > max_chars or 
            gap > pause_threshold or
            (current_end - current_start) > 4.0):  # Max subtitle duration
            ...
            current_text += " " + word_seg['text']
            current_end = word_seg['end']  # Update end time to current word
        
        current_end = word_seg['end']  # Always update end time to current word
...
def create_srt_content_fallback(segments, max_chars=80, min_duration=0.8):
    """
    Fallback method for when word timestamps are not available
    """
    srt_lines = []
    subtitle_number = 1
